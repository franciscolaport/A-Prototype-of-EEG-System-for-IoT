...
for met = 1:4
    for s = 1:length(sujeto)
        ...
        % Leemos los ficheros del sujeto
        [open, closed] = readFiles(sujeto{s}, num_files, placa);

        %% TRAINING
        for k = 1:ciclos
            fprintf('CICLO %d \n', k);
            % Select recordings for training and testing.
            ...
            for d = 1:length(desp)
                %%% DESLIZANTE %%%
                % Obtenemos los indices para los datos de entrenamiento.
                [index_tr_cl, index_tr_op] = featuresDeslizantes(xtr_cl, xtr_op, desp(d), fs, tam_ventana, met);

                % Obtenemos los outliers de los indices calculados.
                ...
                % Training data LDA sin outliers.
            ...
            %% TEST
                ...
                [index_test_cl, index_test_op] = featuresDeslizantes(xtest_cl, xtest_op, desp(d), fs, tam_ventana, met);
